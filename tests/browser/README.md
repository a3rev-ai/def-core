...
| `harness-artifacts.js` | Artifacts A-2 (v8.2.1): an html document opens in a frame that carries exactly `sandbox="allow-scripts"`, its srcdoc built from the content route's text read to the end with the D-A3 CSP meta in front; the Artifacts page lists html by day; the Documents card says ARTIFACT and Open; the console page's CSP carries `frame-src 'self'` and the viewer closes an artifact whose frame loads a second time. v8.2.2 adds the chat's own card: ARTIFACT and Open there too, Open reaching the same viewer, a docx card untouched |
| `harness-artifact-share.js` | Artifacts A-3 (the share link, DEF #1255 / D-A7, D-A4): the BFF forwards `share` and registers both share routes on the sibling document routes' rail; **Share** POSTs and the page shows `{shareOrigin}/a/{token}` built from the origin PHP localised (never a host written into the JS), with Copy and its no-clipboard fallback; **Stop sharing** DELETEs and the page goes back to offering Share; an already-shared artifact opens on its link and an md document after it carries neither; DEF's 409 sentence is what the reader sees; a site with no DEFHO origin is offered no Share; the globe and the lock follow `share` on both lists, each `role="img"` so its words are announced, and neither the token nor a title ever goes in as markup. Read off the stylesheet: the link field paints from tokens declared in BOTH themes and sets its own colour — an undeclared token falls back silently while the colour beside it does not, which is how it reached ~1:1 under `.dark-theme` on round 1 |
...
| `harness-cc-a11y.js` | The Customer Chat panel and greeting bubble from the keyboard and a screen reader (v8.7.1), running the **whole** shipped loader in the page: opening moves focus into the panel — the composer once the chat module has built it, the panel itself while it loads, the panel again when a turn in flight has disabled the composer, and always the panel on a phone so a reopen does not raise the on-screen keyboard; closing by X, Escape, the drawer's backdrop or a click outside hands focus back to the opener (a page trigger, else the launcher), except that a click which put focus on a page field leaves it there. The dialog is named "Chat" and `aria-modal` in all three display modes and `inert` while closed. Tab and Shift+Tab wrap at the panel's ends, bring stray focus back in, and leave the browser's own order alone in between (Ctrl+Tab too). The page-scroll lock goes on `<body>` or `<html>` by where the viewport takes its overflow from, and closing — or `destroy()` — puts the inline style back exactly, but only while the lock is still ours: a theme phone menu that cleared `<body>`'s overflow while the chat was open does not get its old `hidden` written back. Over the shipped chat MODULE in the same page: every icon-only header control is named, reopening focuses the module's own composer, and the message list stays `role="log"` while being a named Tab stop (`tabindex="0"`, `aria-label="Conversation"` — axe scrollable-region-focusable) between Close chat and the composer, which the trap lets through with its ends still wrapping. Since v8.7.3 that list is explicitly `aria-live="off"`: what a screen reader hears is `harness-cc-announce.js`'s. And nothing above it is live: the widget host carries no `aria-live`, and no ancestor of the list — panel, host, page — has `aria-live` or a live role, in any display mode; the loader's hard failure ("Failed to load chat…") is `role="alert"` instead. The greeting bubble is two sibling buttons with nothing interactive nested, opens and dismisses from the keyboard (dismiss lands on the launcher and does not open the chat), and keeps its 24h dismissal |
| `harness-cc-announce.js` | What a screen reader hears of the Customer Chat conversation (v8.7.3), running the **whole** shipped chat module with the shipped marked, DOMPurify, persona and product-card scripts over a fake network — the chat answered as JSON (the sync path) or as an SSE stream fed one event at a time. A MutationObserver on the shadow root from before `init()` counts every node added to **any** live region, not just the announcer, so a live log is caught speaking. The log is `role="log"` and explicitly `aria-live="off"`; within the module the one live region is a visually hidden announcer, last in the panel, built empty. Its scope is the module: the test host has no `aria-live` and a node's region is found by Chrome's rule (nearest `aria-live` wins, `off` ends the walk), so a live region outside the module is `harness-cc-a11y.js`'s to catch. Startup (banner, greeting, chips, a site's opening message, the offline notice), a local thread restored, a server thread restored on a logged-in load or adopted after logging in, the logged-out-elsewhere reset and New conversation add nothing to it. A new message is said once, as the words its bubble shows (markdown rendered, a `<br>` and blocks as breaks, an image as its alt, never markup): the visitor's (typed, a chip, spoken once its transcript fills the bubble), a non-streamed reply, a streamed reply only when the stream ends — `done`, `error`, or cut off — never its start, its tokens, a superseded round or a tool row; and each notice raised mid-conversation (errors, the rate limit, the cart's toast and link, Welcome back, logged out, Conversation ended). Product cards are drawn and not said; a bubble cleared while it streamed says nothing when its `done` lands; each turn begins by emptying the announcer (removals, which are never spoken); no line is written twice or edited |
...
CC_LOADER=/tmp/old-loader.js CC_MODULE=/tmp/old-chat.js node tests/browser/harness-cc-a11y.js  # the chat loader, whole / the chat module
CC_MODULE=/tmp/old-chat.js    node tests/browser/harness-cc-announce.js  # the chat module, whole (8.7.2 fails 31 of 32)
```
