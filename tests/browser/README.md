...
| `harness-page-back.js` | The "‹ Back" control (v7.9.16, corrected in v7.9.17): every shipped page carries it; it pops the console's own history entries — an open document back to Documents, a page back to the chat — and falls back to the chat from an entry that is not the console's (a reload, the browser's Back), never leaving the console |
