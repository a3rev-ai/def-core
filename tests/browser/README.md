...
| `harness-artifacts.js` | Artifacts A-2 (v8.2.1): an html document opens in a frame that carries exactly `sandbox="allow-scripts"`, its srcdoc built from the content route's text read to the end with the D-A3 CSP meta in front; the Artifacts page lists html by day; the Documents card says ARTIFACT and Open |
