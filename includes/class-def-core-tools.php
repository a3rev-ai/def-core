...
final class DEF_Core_Tools {
	...
	public static function wc_add_to_cart( \WP_REST_Request $req ): \WP_REST_Response {
		...
		try {
			...
			// Clear any pre-existing WooCommerce notices so we only capture
			// notices generated by THIS add_to_cart call (not stale notices
			// from a previous request that happened to share the session).
			...
			// Capture WooCommerce's error notices. add_to_cart() catches its
			// own exceptions and forwards the human message into wc_add_notice
			// (e.g. "Sorry, you cannot add another \"X\" to your cart.",
			// "You cannot add that amount — we have N in stock and you already
			// have M in your cart."). These messages are the natural-language
			// signal we want to give back to the LLM instead of a generic
			// "Failed to add product to cart".
			$wc_error_messages = array();
			if ( function_exists( 'wc_get_notices' ) ) {
				foreach ( wc_get_notices( 'error' ) as $_n ) {
					$_msg = is_array( $_n ) ? ( $_n['notice'] ?? '' ) : (string) $_n;
					if ( '' === $_msg ) {
						continue;
					}
					// WooCommerce wraps several error messages with an
					// inline "View cart" anchor — e.g. "Sorry, you cannot
					// add another \"X\" to your cart. <a …>View cart</a>".
					// After wp_strip_all_tags() the "View cart" word is left
					// dangling on the end ("…to your cart. View cart"),
					// which reads as broken English in chat. Strip the
					// anchor first, then strip the rest of the tags, so the
					// final message is just "Sorry, you cannot add another
					// \"X\" to your cart.".
					$_msg = preg_replace(
						'#<a\b[^>]*\bclass=["\'][^"\']*\bwc-forward\b[^"\']*["\'][^>]*>.*?</a>#is',
						'',
						$_msg
					);
					$_msg = trim( wp_strip_all_tags( $_msg ) );
					if ( '' !== $_msg ) {
						$wc_error_messages[] = $_msg;
					}
				}
				wc_clear_notices();
			}
			...
			if ( ! $cart_item_key ) {
				return new \WP_REST_Response(
					array(
						'error'             => true,
						'message'           => $wc_error_messages
							? implode( ' ', $wc_error_messages )
							: 'Failed to add product to cart',
						'wc_error_messages' => $wc_error_messages,
					),
					400
				);
			}
			...
			$response_data = array(
				'success'       => true,
				'message'       => '' !== $success_message ? $success_message : 'Product added to cart',
