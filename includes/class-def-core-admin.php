...
final class DEF_Core_Admin {
	...
	public static function enqueue_setup_assistant_drawer(): void {
		...
		wp_localize_script( 'def-core-setup-assistant', 'defSetupAssistant', array(
			...
			'activeThreadUrl' => esc_url_raw( rest_url( DEF_CORE_API_NAME_SPACE . '/setup-assistant/active-thread' ) ),
	 ...
	 *
	 * No truncation. The REST route (Setup Assistant) refuses over-length input with a visible error; the admin settings form bounds it with a maxlength attribute. Neither cuts silently.
	...
	public static function ajax_save_user_roles(): void {
		...
		foreach ( $roles_data as $user_id => $caps ) {
			...
			$has_staff = isset( $caps['def_staff_access'] ) && '1' === $caps['def_staff_access'];
			$has_mgmt  = isset( $caps['def_management_access'] ) && '1' === $caps['def_management_access'];
			if ( $has_staff && $has_mgmt ) {
