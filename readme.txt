...
* Security: Legal Link URL sanitiser hardened. The stored value flows into the `href` of admin-controlled links; the sanitiser now restricts absolute URLs to http/https, requires both scheme and host, allows same-origin paths beginning with `/`, and caps length at 2048 characters. Invalid input drops to empty string. Defence-in-depth — `javascript:` and `data:` were already blocked by WP's default allowlist; the tightened rule makes the privacy-link surface inert against future allowlist drift.
