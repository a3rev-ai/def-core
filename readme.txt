...
= 8.2.2 - 2026-09-16 =
* Fix - A new release now appears in WordPress when it is published, instead of up to twelve hours later. The plugin remembers the latest release for twelve hours so it does not hammer GitHub, but nothing ever cleared that memory: on a day when four releases went out, updating to one of them refilled it with the version you had just installed, and your sites could not see the next one until the following morning - the zip had to be uploaded by hand. Updating this plugin now forgets the remembered release straight away, and "Check again" on Dashboard → Updates asks GitHub afresh.
