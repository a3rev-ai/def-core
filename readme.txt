...
= 2.7.0 - 2026-04-28 =
* Feature: welcome-state suggestion chips. Up to three admin-configurable opening prompts (Chat Settings → Welcome State) render as tappable pills below the greeting in Customer Chat. Tap pre-fills the composer and submits, mirroring the Bunnings "Buddy" / Intercom Resolution Bot pattern. Empty admin slots are hidden — no chip slots show if none are configured.
* Feature: compliance footer with two configurable fields. The Notice Text (lead sentence, max 500 chars) defaults to *"AI content may be inaccurate. In using our AI you agree to our"*; the Legal Link Label (trailing link words, max 50 chars) defaults to *"Terms and Conditions"*. Together the default footer reads: *"AI content may be inaccurate. In using our AI you agree to our Terms and Conditions"* — clickable when a Legal Link URL is configured, plain text otherwise. Both fields can be edited independently to suit any phrasing the tenant prefers.
* Feature: one-tap refresh button in the chat header. Same handler as the existing "Clear chat" menu item but reachable in one click instead of two. The 3-dot menu's "Clear chat" option remains unchanged.
* Polish: text-readability pass. Body messages bumped from 14px → 15px (16px in Spotlight mode), composer input bumped to 16px (prevents iOS Safari from zooming the viewport on focus), AI disclosure notice bumped from 11px → 12px, line-height bumped from 1.5 → 1.6 in messages. Visible across all display modes.
* Feature: configurable Legal Link Label. The "Privacy Policy URL" field is renamed "Legal Link URL" and gains a sibling "Legal Link Label" text field — admins can now set the displayed link text to "Terms and Conditions", "Legal", or anything else (max 50 chars). Defaults to "Privacy Policy" when empty so existing tenants see no change.
* Behavior: clearing the Compliance Footer Text now hides the entire compliance footer — including the Legal Link Label. The label is the trailing words of the lead sentence and has no meaning standalone, so both fields hide together when an admin clears Notice Text. (Smoke-test fix.)
* Behavior: AI Disclosure notice and Compliance Footer both render the Legal Link Label as a clickable link whenever a Legal Link URL is configured. The two surfaces are independent and each links its own label. (Smoke-test fix — the footer label previously rendered as plain text whenever the AI notice was on.)
* Security: Legal Link URL sanitiser hardened. The stored value flows into the `href` of admin-controlled links; the sanitiser now restricts absolute URLs to http/https, requires both scheme and host, allows same-origin paths beginning with `/` (rejecting protocol-relative `//evil.com` which would otherwise render as a cross-origin link), and caps length at 2048 characters. Invalid input drops to empty string. Defence-in-depth — `javascript:` and `data:` were already blocked by WP's default allowlist; the tightened rule closes a phishing-link surface and makes the privacy-link inert against future allowlist drift.
* Security: client-side scheme allowlist applied at the DOM sink (immediately before assigning to `<a>.href`) as a second layer over the PHP sanitiser. A bad URL falls through to plain-text rendering instead of attaching a hostile href. No behaviour change for legitimate URLs.
* Fix: whitespace-only Compliance Footer Text now collapses to empty, so the footer hides as expected. Previously a value of e.g. " " (just a space) bypassed the JS empty-string guard and rendered an empty lead with a dangling Legal Link Label.
* Internal: five new tenant-config options registered in the admin API + Setup Assistant settings registry (`def_core_chat_welcome_chip_1`–`_3`, `def_core_chat_compliance_text`, `def_core_chat_privacy_link_label`). Same sanitiser/validator pattern as existing options.
