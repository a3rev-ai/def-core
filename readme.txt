...
= 8.6.0 - 2026-09-23 =
* New Feature - Opening message: Chat Settings has a new Opening message field, the first message visitors see when the chat opens, in your assistant's own words. Line breaks are preserved. Leave it blank and the chat opens with the built-in greeting as before.
...
= 8.2.6 - 2026-09-16 =
* Tweak - Content AI: the page is Carol's — its title reads "Carol - Creator" (your own name for her if you renamed her in the tenant portal) and the copy says Carol instead of "the Content Agent", matching the tenant portal.
...
= 7.8.3 - 2026-09-09 =
* Update - Staff AI: Memories, Usage and Connections are now pages in the console, like Documents, Scheduled and Projects, instead of boxes over your chat. Each has its own address, so the browser's back button, a phone's back gesture and a reload behave as they do on any site, and the sidebar shows which one you are on - all six entries are now places you go, not boxes that open. The rows, bars and cards on each are exactly as they were, with room to grow down the page instead of scrolling inside a box. Weekly limits keeps its Refresh, because the numbers move while you read them; Memories and Connections load fresh every time you open them, so their Refresh buttons are gone. Deleting a memory, disconnecting an account and choosing your primary mailbox all still ask before they act, exactly as before. Each page carries an Ask how it works entry, so your assistant can explain it in the chat rather than the page explaining itself. Your chat is hidden while a page is open, never reset: a reply that is streaming keeps streaming, and a half-typed message is still in the box when you return.
...
* Tweak - Customer Chat hand-off form: optional Business name field.
...
= 7.3.0 - 2026-09-01 =
* Feature - Staff AI. Projects now start in the chat: the Projects panel has an "Ask <your assistant> how Projects work" button that opens a conversation in which the assistant explains projects step by step and can create one for you when you say so (it opens the new project in that chat and starts it with a skeleton runsheet and instructions for you to fill together). Every project row now shows its three documents - runsheet, session notes, instructions - and whether each exists, without a click.
...
* Tweak - Each task card now says its real rhythm - "Every hour at ~:30", "Weekdays at ~7:00 AM", "Every Friday at ~7:00 AM", or "Runs when you press Run now"
...
= 5.9.0 - 2026-08-16 =
* Improvement: Documents, Scheduled, Memories and Connections now sit in the Staff AI sidebar as named menu items instead of unlabelled icons in the header - you can see what they are without hovering over each one to find out.
...
* Security: hardened the Staff AI upload area against a crafted-filename issue.

= 5.8.3 - 2026-08-12 =
* Improvement: upload failures no longer lose your work. If attached files fail to upload, sending is refused with the reason shown - Staff AI previously sent the message without its files when every attachment had failed, and Customer Chat refused silently. Customer Chat now keeps your typed message when an upload fails, and shows the server's own explanation (including how long to wait when rate limited) instead of a generic error.

= 5.8.2 - 2026-08-12 =
* Staff AI: the schedule card's "Saved." status now renders as a green success pill (both themes) instead of a plain grey line
...
= 5.7.12 - 2026-08-11 =
* Dev: the plugin's WordPress-integration test suite (security gates, route registration, JWT bridge) now runs on every change - five test files that had never executed in CI are green and gating. Three stale tests were fixed on the way in, including one whose pass/fail depended on network accidents rather than the code.
...
* Fix: when the assistant speaks, uses a tool, then speaks again, the two parts of its reply no longer run together into one another ("...right now.Done — I've flagged this"). They are now separated as distinct paragraphs, as they were written.
* Fix: on a phone the hand-off form was taller than the panel, leaving the Send button below the fold with nothing to suggest scrolling. The Cancel/Send row is now pinned to the bottom of the form so it is always reachable.
...
= 5.5.6 - 2026-07-28 =
* Tweak: removed an inert mobile font-size rule from the Customer Chat stylesheet and corrected its documentation. Investigating the "restore the mobile readability bump" follow-up showed the bump needs no restoring: since v2.7.0 the message text has carried an explicit 15px size at every viewport (themable as --def-cc-font-size-message since 5.5.5), and no element in the widget inherits the panel-level font size the mobile rule was setting — so the rule affected nothing, and was additionally shadowed by the boot stylesheet's style order. The rule, and the comment that wrongly claimed reviving it would change mobile rendering, are removed. No visual change.
...
= 5.5.2 - 2026-07-28 =
* Feature: Fourteen more Customer Chat theming tokens (theming contract, conflation cleanup). Surfaces that shared one general-purpose token now each carry their own: the markdown table header, the staged-attachments tray, the tool-status pill and the welcome-banner backdrop split off --def-cc-bg-alt — which now drives nothing directly, though setting it still tints every surface that defaults to it (these four plus the header, footer and welcome chips split off it in 5.5.0). Form labels, overflow-menu items and file chips get --def-cc-text-form (the mid-grey previously hardcoded in five places); the attachment-error tooltip gets --def-cc-tooltip-bg/-text (it previously borrowed the body-text colour as its background, so a theme lightening the text would have washed out the tooltip); disabled buttons get --def-cc-disabled-bg/-text (previously the muted-text and input-border tokens doing double duty); the composer, login and escalation inputs get --def-cc-input-bg/-text (previously welded to the panel surface, so a dark-panel brand could not keep light inputs); the escalation form's Cancel button gets --def-cc-btn-secondary-bg/-hover-bg; and the in-message file chip's hardcoded white becomes --def-cc-file-chip-bg. Every new token defaults to the exact value it split from, so nothing changes visually until a theme overrides one.

= 5.5.1 - 2026-07-27 =
* Feature: Skin the Customer Chat widget with CSS Shadow Parts. Version 5.5.0 made the widget's colours themable; this release makes its structure targetable. Every element the widget builds now carries a `part` name — including the launcher icon image, the greeting-bubble logo and the product-card image, each of which was previously reachable only by a descendant rule no theme could express — so a theme can style them directly with #def-customer-chat-host::part(...) — descendant selectors cannot cross a Shadow DOM boundary and never could, which is why the widget's internals have been unreachable until now. Around 145 parts cover the header and its controls, the message stream and both bubble types, the composer, attachments, product result cards, speaker dividers, the menu, the login overlay, and the entire email-escalation form including its validation-error, sending, success and failure states. Part names are derived from the class name with the def-cc- prefix stripped (def-cc-message--user becomes message--user), so the two cannot drift apart. State is targetable too. The host carries data-open while the chat is open, and that is all it means: the backdrop and launcher-hidden states apply only in drawer mode and the mobile state only below 480px, so each of those carries its own part (backdrop--visible, trigger--hidden, panel--mobile-open) rather than being inferred from data-open. Per-element states such as message--streaming and escalation-input--error are added to and removed from the part list as they change. The escalation success and failure screens were previously built from inline styles that no theme could override; they are now classed elements. One small visual change: the greeting bullet list no longer carries its own indent and now matches every other list in the widget (about 4px wider). Known limit: ::part() cannot be followed by a descendant selector, so markdown-rendered message content stays on the CSS custom properties from 5.5.0.
...
= 4.13.3 - 2026-06-20 =
* Feature: On desktop (viewport > 782px) the Setup Assistant is now a persistent always-open docked sidebar on the Settings and Content AI pages. The trigger button is hidden on desktop and the page content reflows into the remaining width. Mobile (≤782px) keeps the existing trigger → full-width drawer behaviour unchanged.

= 4.13.2 - 2026-06-19 =
* Tweak: Staff-AI now uses the canonical DEF chat bubble style.

= 4.13.1 - 2026-06-19 =
* Tweak: Customer Chat user bubble now follows the tenant brand colour.
