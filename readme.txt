...
= 5.5.0 - 2026-07-27 =
* Feature: Theme the Customer Chat widget to your brand (theming contract, Tier 1). The chat renders inside a Shadow DOM, so page CSS cannot reach its internals — only CSS custom properties cross that boundary. Seventeen new design tokens make the previously unreachable surfaces themable: the header (background, text, border — it previously shared one token with six unrelated surfaces, so colouring it discoloured the staged tray, tool-status pills, chips, footer and banner too), the message canvas (was hardcoded #edf0f5), both message bubbles (background, text, border, corner and tail radius), markdown link colour, the compliance footer, welcome chips, and two new radius steps. Set any of them from your theme with a rule on the host element, e.g. #def-customer-chat-host { --def-cc-header-bg: #0E403C; }. The launcher colour set in Branding still flows through the whole widget, but is now injected as --def-cc-brand and derived into --def-cc-primary/--def-cc-btn-color in the stylesheet rather than being written inline on the host — an inline value beat every stylesheet rule, so theming it was impossible. The loader's inline stylesheet (which wins the cascade over the main stylesheet) now reads the same tokens, so panel background, font, text colour, shell radius and shadow are genuinely themable rather than silently ignored. Focus rings gained a static fallback so an invalid brand colour can no longer make them disappear.
